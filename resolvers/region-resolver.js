  ...
  Mutation: {
    addRegion: async (_, args) => {
      ...
      if (updateParent_Id === "root") {
        ...
        const parentUpdate = await Region.updateOne(
          { _id: checkId },
          { childRegionIds: newArray }
        );
    ...
    deleteRegion: async (_, args) => {
      ...
      if (updateParent_Id === "root") {
        ...
        if (parent) {
          ...
          const updateParent = await Region.updateOne(
            { _id: updateParent_Id },
            { childRegionIds: childRegionArray }
          );
    ...
    changeParent: async (_, args) => {
      ...
      const found = await Region.findOne({ _id: objectId });
      if (found) {
        const update = await Region.updateOne(
          { _id: objectId },
          { parentId: newParent_id }
        );
        ...
        const updateOld = await Region.updateOne(
          { _id: objectId2 },
          { childRegionIds: childs }
        );

        const newParent = await Region.findOne({ _id: objectId3 });
        let childs2 = newParent.childRegionIds;
        childs2.push(_id);
        const updateNew = await Region.updateOne(
          { _id: objectId3 },
          { childRegionIds: childs2 }
        );
